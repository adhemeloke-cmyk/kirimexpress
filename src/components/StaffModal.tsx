...
import { X, Lock, CheckCircle, Trash2, KeyRound, Ship, Info } from 'lucide-react';
...
export const StaffModal: React.FC<StaffModalProps> = ({
  ...
  const handlePublishSchedule = (e: React.FormEvent) => {
    ...
    const newSched: ShipSchedule = {
      ...
      status: schedStatus,
      notes: schedNotes.trim() ? schedNotes.trim() : undefined,
    };
          ...
          <div className="py-4 space-y-4">
            <div className="text-center">
              ...
              <h3 className="text-lg font-extrabold text-slate-900">
                {lang === 'zh' ? '员工/管理员登录 Staff Login' : 'Login Staff NK EXPRESS'}
              </h3>
          ...
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <div>
                ...
                <p className="text-[10px] text-red-600 font-medium">Logged in as Admin NK EXPRESS</p>
              </div>
              <button
                onClick={onLogout}
                ...
                <form onSubmit={handlePublishSchedule} className="space-y-3 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                  ...
                  <div>
                    <label className="block text-slate-600 mb-1 font-semibold flex items-center justify-between">
                      <span>{lang === 'zh' ? 'Note (可选)' : 'Note (Opsional)'}</span>
                      <span className="text-[10px] text-slate-400 font-normal">{lang === 'zh' ? '展示在官网' : 'Tampil di jadwal publik'}</span>
                    </label>
                    <input
                      type="text"
                      value={schedNotes}
                      onChange={(e) => setSchedNotes(e.target.value)}
                      placeholder={lang === 'zh' ? '例如：Dermaga Bastiong, 大件行李需提前确认...' : 'Contoh: Sandar di Pelabuhan Bastiong, muatan sembako & karung prioritas'}
                      className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-slate-900 placeholder-slate-400 focus:outline-none focus:border-red-500 text-xs"
                    />
                  </div>
                  ...
                  {schedules.map((sc) => (
                    <div key={sc.id} className="bg-slate-50 p-3 rounded-xl border border-slate-200 flex items-center justify-between">
                      <div className="flex-1 pr-2">
                        ...
                        {sc.notes && (
                          <div className="mt-1 flex items-start gap-1 text-[10px] text-amber-800 bg-amber-50/80 border border-amber-200/70 rounded px-1.5 py-0.5">
                            <Info className="w-3 h-3 text-amber-600 shrink-0 mt-0.5" />
                            <span><strong className="text-amber-900">Note:</strong> {sc.notes}</span>
                          </div>
                        )}
                      </div>
                      <button
                        onClick={() => onDeleteSchedule(sc.id)}
                        className="p-1.5 text-red-600 hover:text-red-700 bg-red-50 hover:bg-red-100 rounded-lg border border-red-200 cursor-pointer transition-colors shrink-0"
                        title="Hapus Jadwal"
                      >
