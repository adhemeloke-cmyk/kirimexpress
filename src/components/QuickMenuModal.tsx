...
import { X, MapPin, Phone, Lock, Building2 } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-sm p-5 text-left text-slate-800 shadow-2xl relative">
        ...
        <h3 className="text-base font-extrabold text-slate-900 flex items-center gap-2 mb-3">
          ...
          <span>{lang === 'zh' ? '仓库地址与服务网络' : 'Lokasi Gudang & Fitur NK EXPRESS'}</span>
        </h3>
        ...
        <div className="space-y-3 text-xs">
          ...
          <div className="bg-slate-50 border border-red-200 p-3 rounded-xl">
            <span className="text-[10px] font-bold text-red-600 uppercase tracking-wider flex items-center gap-1">
              ...
              {lang === 'zh' ? WAREHOUSE_ADDRESSES.jakarta.titleZh : WAREHOUSE_ADDRESSES.jakarta.title}
            </span>
            <h4 className="font-bold text-slate-900 text-xs sm:text-sm mt-0.5">{WAREHOUSE_ADDRESSES.jakarta.name}</h4>
            <p className="text-slate-600 mt-1 leading-relaxed text-[11px]">
              {WAREHOUSE_ADDRESSES.jakarta.fullAddress}
            </p>
            <p className="text-emerald-600 font-semibold mt-1 flex items-center gap-1 text-[11px]">
              <Phone className="w-3 h-3 shrink-0" />
              <span>CS WhatsApp:{' '}</span>
              <a
                href={`https://wa.me/${WAREHOUSE_ADDRESSES.jakarta.rawPhone}?text=${encodeURIComponent('Halo NK EXPRESS, saya ingin bertanya tentang layanan pengiriman paket.')}`}
                target="_blank"
                rel="noopener noreferrer"
                className="font-bold underline hover:text-emerald-700 transition-colors cursor-pointer"
              >
                {WAREHOUSE_ADDRESSES.jakarta.phone}
              </a>
            </p>
          </div>
          ...
          <div className="bg-slate-50 border border-red-200 p-3 rounded-xl">
            <span className="text-[10px] font-bold text-red-600 uppercase tracking-wider flex items-center gap-1">
              ...
              {lang === 'zh' ? WAREHOUSE_ADDRESSES.ternate.titleZh : WAREHOUSE_ADDRESSES.ternate.title}
            </span>
            <h4 className="font-bold text-slate-900 text-xs sm:text-sm mt-0.5">{WAREHOUSE_ADDRESSES.ternate.name}</h4>
            <p className="text-slate-600 mt-1 leading-relaxed text-[11px]">
              {WAREHOUSE_ADDRESSES.ternate.fullAddress}
            </p>
            <p className="text-emerald-600 font-semibold mt-1 flex items-center gap-1 text-[11px]">
              <Phone className="w-3 h-3 shrink-0" />
              <span>CS WhatsApp:{' '}</span>
              <a
                href={`https://wa.me/${WAREHOUSE_ADDRESSES.ternate.rawPhone}?text=${encodeURIComponent('Halo NK EXPRESS, saya ingin bertanya tentang layanan pengiriman paket.')}`}
                target="_blank"
                rel="noopener noreferrer"
                className="font-bold underline hover:text-emerald-700 transition-colors cursor-pointer"
              >
                {WAREHOUSE_ADDRESSES.ternate.phone}
              </a>
            </p>
          </div>
